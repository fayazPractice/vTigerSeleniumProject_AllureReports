...
{
	@Test(priority = 1, description = "verifying Create Contact With Org And Delete With Org test")
	@Severity(SeverityLevel.MINOR)
	@Description("Test Case Description: Verify Create Contact With Org And Delete With Org test on Login Page")
	@Story("Story Name: To check Create Contact With Org And Delete With Org")
	public void CreateContactWithOrgAndDeleteWithOrg() throws Throwable
	{
		...
		CreatingNewOrganization.getAccountname().sendKeys(data.FetchDataFromExcelFile("Contacts", 13, 3),"_"+driverScripts.getRamDomNum());
		/* Step 4: Select Industry from DropDown */
		driverScripts.SelectDropDownByValue(CreatingNewOrganization.getIndustry(), data.FetchDataFromExcelFile("Contacts", 14, 3));
		/* Step 5: Select AccountType from DropDown */
		driverScripts.SelectDropDownByValue(CreatingNewOrganization.getAccounttype(), data.FetchDataFromExcelFile("Contacts", 15, 3));
		...
		CreatingNewContact.getLastname().sendKeys(data.FetchDataFromExcelFile("Contacts", 19, 3)+driverScripts.getRamDomNum());
		...
		driverScripts.SwitchToSpecificWindow(driver,data.FetchDataFromExcelFile("Contacts", 20, 3));
		...
		driverScripts.SwitchToSpecificWindow(driver,data.FetchDataFromExcelFile("Contacts", 22, 3));
		...
		driverScripts.waitForElementVisibility(driver, HomePage.getContactsLink());
		...
		driverScripts.waitForElementVisibility(driver, Contacts.getInventoryCount());
		...
		driverScripts.waitForElementVisibility(driver, HomePage.getOrganisationsLink());
		...
		driverScripts.waitForElementVisibility(driver, Oganisations.getInventoryCount());
